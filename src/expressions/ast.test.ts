...
describe("AST Node Interfaces", () => {
	...
	it("should define Getter node correctly", () => {
		const node: AST.Getter = {
			type: "Getter",
			receiver: { type: "ID", value: "obj" },
			name: "prop",
			optional: true,
		};
	...
	it("should define SpreadProperty node correctly", () => {
		const node: AST.SpreadProperty = {
			type: "SpreadProperty",
			expression: { type: "ID", value: "obj" },
		};
