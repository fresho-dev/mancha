...
describe("Parser", () => {
	...
	it("should respect precedence", () => {
		assert.deepEqual(
			...
			factory.binary(
				factory.literal(1),
				"+",
				factory.binary(factory.literal(2), "*", factory.literal(3)),
			),
		);
		assert.deepEqual(
			parseExpr("(1 + 2) * 3"),
			factory.binary(
				factory.paren(factory.binary(factory.literal(1), "+", factory.literal(2))),
				"*",
				factory.literal(3),
			),
		);
	...
	it("should parse method invocation", () => {
		assert.deepEqual(
			parseExpr("obj.method()"),
			factory.invoke(factory.id("obj"), "method", [], false),
		);
		assert.deepEqual(
	...
	it("should parse optional chaining", () => {
		...
		assert.deepEqual(
			parseExpr("obj?.method()"),
			factory.invoke(factory.id("obj"), "method", [], true),
		);
		assert.deepEqual(
			parseExpr("func?.()"),
			factory.invoke(factory.id("func"), undefined, [], true),
		);
		assert.deepEqual(
			parseExpr("arr?.[0]"),
			factory.index(factory.id("arr"), factory.literal(0), true),
		);
	});
	...
	it("should parse object literals with shorthand properties", () => {
		assert.deepEqual(parseExpr("{obj}"), factory.map([factory.property("obj", factory.id("obj"))]));
		assert.deepEqual(
			parseExpr("{a, b}"),
			factory.map([factory.property("a", factory.id("a")), factory.property("b", factory.id("b"))]),
		);
	});
	...
	it("should parse typeof expressions", () => {
		...
		assert.deepEqual(
			...
			factory.binary(factory.unary("typeof", factory.id("x")), "===", factory.literal("string")),
		);
	});
