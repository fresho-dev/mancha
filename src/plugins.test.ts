		...
		describe(":for", () => {
			...
			describe(":key reconciliation", () => {
				...
				it("stops waking rows that were dropped without being disposed", async () => {
					// Nested :for builds a subrenderer per inner row and never disposes it. A
					// subscription outliving the row would make every later mutation reach further
					// than the last, with nothing but a slowdown to show for it.
					...
					try {
						...
						// A subscription belonging to a dropped row is dropped when it is next
						// woken, so the first mutation after the churn still pays for the rows it
						// is clearing out. Steady-state cost is what has to stay flat, so measure
						// the mutation after that one. A subscription that really outlived its row
						// would keep being woken and this would not come back down.
