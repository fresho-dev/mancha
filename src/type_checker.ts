...
function getAttributeValueRange(
	...
	if (equalsIndex === -1) {
		return {
			...
			length: attrLocation.endOffset - attrLocation.startOffset,
		};
	}
	...
	try {
		...
		if (options.debug) {
			...
			for (const [offset, entry] of expressionMap.entries()) {
				console.log(
					`  TS offset ${offset}: "${entry.expression}" -> HTML range ${JSON.stringify(entry.range)}`,
				);
			}
		...
		for (const diag of rawDiagnostics) {
			...
			if (bestMatch && bestMatch.entry.range) {
				...
				if (options.debug) {
					...
					console.log(
						`  newStart=${newStart}, newLength=${newLength}, in bounds: ${newStart < html.length}`,
					);
					console.log(`  Message: ${msg}`);
				}
...
const TRUSTED_DATA_ATTRIBUTE_SET = new Set(TRUSTED_DATA_ATTRIBS.map((attr) => attr.toLowerCase()));
...
function getExpressionsExcludingNestedTypes(
	root: Element,
	dom: JSDOM,
	html: string,
): ExpressionScope {
	...
	const processElement = (element: Element, currentScope: ExpressionScope) => {
		...
		if (forAttr) {
			...
			currentScope.forLoops.push({
				itemName,
				itemsExpression: itemsExpressionEntry,
				scope: forScope,
			});
		} else {
			// Process attributes
			for (const attr of Array.from(element.attributes)) {
				if (isExpressionAttribute(attr.name)) {
					currentScope.expressions.push({
						...
						range: getAttributeValueRange(element, attr.name, attr.value, attr.value, dom, html, 0),
					});
				}
			}
